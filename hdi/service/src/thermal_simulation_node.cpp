...
const int ARG_3 = 3;
const int ARG_4 = 4;
std::string thermalDir = "/data/thermal";
std::string thermalNodeDir = "/data/thermal/%s";
std::string thermalFileDir = "%s/%s";
std::string thermalTypeDir = "/data/thermal/%s/type";
std::string thermalTempDir = "/data/thermal/%s/temp";
std::string mitigationDir = "/data/mitigation";
std::string mitigationNodeDir = "/data/mitigation/%s";
...
int32_t ThermalSimulationNode::CreateNodeDir(std::string dir)
{
    ...
    if (access(dir.c_str(), 0) != HDF_SUCCESS) {
        ...
        if (flag == HDF_SUCCESS) {
            HDF_LOGD("%{public}s: Create directory successfully.", __func__);
        } else {
...
int32_t ThermalSimulationNode::CreateNodeFile(std::string filePath)
{
    ...
    if (access(filePath.c_str(), 0) != 0) {
        ...
        HDF_LOGD("%{public}s: the file already exists.", __func__);
    }
    return HDF_SUCCESS;
}
...
int32_t ThermalSimulationNode::AddSensorTypeTemp()
{
    ...
    for (auto dir : sensor) {
        snprintf_s(nodeBuf, PATH_MAX, sizeof(nodeBuf) - ARG_1, thermalNodeDir.c_str(), dir.first.c_str());
        HDF_LOGD("%{public}s: node name: %{public}s", __func__, nodeBuf);
        CreateNodeDir(static_cast<std::string>(nodeBuf));
        for (auto file : vFile) {
            snprintf_s(fileBuf, PATH_MAX, sizeof(fileBuf) - ARG_1, thermalFileDir.c_str(), nodeBuf, file.c_str());
            HDF_LOGD("%{public}s: file name: %{public}s", __func__, fileBuf);
            CreateNodeFile(static_cast<std::string>(fileBuf));
        }
        snprintf_s(typeBuf, PATH_MAX, sizeof(typeBuf) - ARG_1, thermalTypeDir.c_str(), dir.first.c_str());
        std::string type = dir.first + "\n";
        WriteFile(typeBuf, type, type.length());
        snprintf_s(tempBuf, PATH_MAX, sizeof(tempBuf) - ARG_1, thermalTempDir.c_str(), dir.first.c_str());
...
int32_t ThermalSimulationNode::AddMitigationDevice()
{
    ...
    for (auto dir : vSensor) {
        snprintf_s(nodeBuf, PATH_MAX, sizeof(nodeBuf) - ARG_1, mitigationNodeDir.c_str(), dir.c_str());
        HDF_LOGD("%{public}s: node name: %{public}s", __func__, nodeBuf);
        CreateNodeDir(static_cast<std::string>(nodeBuf));
        vFile.push_back(nodeBuf);
    }
    snprintf_s(fileBuf, PATH_MAX, sizeof(fileBuf) - ARG_1, mitigationNodeFileDir.c_str(), vFile[ARG_0].c_str(),
        cpu.c_str());
    CreateNodeFile(static_cast<std::string>(fileBuf));
    WriteFile(fileBuf, sTemp, sTemp.length());
    snprintf_s(fileBuf, PATH_MAX, sizeof(fileBuf) - ARG_1, mitigationNodeFileDir.c_str(), vFile[ARG_1].c_str(),
        charger.c_str());
    CreateNodeFile(static_cast<std::string>(fileBuf));
    WriteFile(fileBuf, sTemp, sTemp.length());
    snprintf_s(fileBuf, PATH_MAX, sizeof(fileBuf) - ARG_1, mitigationNodeFileDir.c_str(), vFile[ARG_2].c_str(),
        gpu.c_str());
    CreateNodeFile(static_cast<std::string>(fileBuf));
    WriteFile(fileBuf, sTemp, sTemp.length());
    for (auto b : vBattery) {
        snprintf_s(fileBuf, PATH_MAX, sizeof(fileBuf) - ARG_1, mitigationNodeFileDir.c_str(), vFile[ARG_3].c_str(),
            b.c_str());
        CreateNodeFile(static_cast<std::string>(fileBuf));
        WriteFile(fileBuf, sTemp, sTemp.length());
    }
...
int32_t ThermalSimulationNode::SetTempRequest(std::string type, int32_t temp)
{
    HDF_LOGD("%{public}s: enter", __func__);
    char tempBuf[MAX_PATH] = {0};
    snprintf_s(tempBuf, PATH_MAX, sizeof(tempBuf) - ARG_1, thermalTempDir.c_str(), type.c_str());
    std::string sTemp = std::to_string(temp) + "\n";
