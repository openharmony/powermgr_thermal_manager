...
MessageParcel::MessageParcel(Allocator* allocator) :
    Parcel(allocator), writeRawDataFd_(-1), readRawDataFd_(-1), kernelMappedWrite_(nullptr), kernelMappedRead_(nullptr),
    rawData_(nullptr), rawDataSize_(0)
{
}
...
{
    (void)object;
#ifdef MOCK_WRITE_REMOTE_OBJECT_RETURN_FALSE
    return false;
#else
    return true;
#endif
...
{
    (void)name;
#ifdef MOCK_WRITE_INTERFACE_TOKEN_RETURN_FALSE
    return false;
#else
    return true;
#endif
