...
namespace THERMAL {
namespace V1_0 {
ThermalHDIConfigParser &ThermalHDIConfigParser::GetInsance()
{
    static ThermalHDIConfigParser instance;
    return instance;
}

int32_t ThermalHDIConfigParser::ThermalHDIConfigInit(const std::string &path)
{
...
std::map<std::string, uint32_t> ThermalHDIConfigParser::GetIntervalMap()
{
    return intervalMap_;
}

ThermalHDIConfigParser::ThermalTypeMap ThermalHDIConfigParser::GetSensorTypeMap()
{
    return typesMap_;
}

int32_t ThermalHDIConfigParser::ParseThermalHdiXMLConfig(const std::string &path)
{
    HDF_LOGD("%{public}s: enter", __func__);
    ...
    if (!xmlStrcmp(rootNode->name, BAD_CAST"thermal")) {
        ...
        HDF_LOGD("%{public}s: version: %{public}s, product: %{public}s",
...
void ThermalHDIConfigParser::ParseBaseNode(xmlNodePtr node)
{
    HDF_LOGD("%{public}s: enter", __func__);
    ...
    while (cur != nullptr) {
        ...
        HDF_LOGD("%{public}s: ParseBaseNode tag: %{public}s, value: %{public}s",
...
void ThermalHDIConfigParser::ParsePollingNode(xmlNodePtr node)
{
    HDF_LOGD("%{public}s: enter", __func__);
    auto cur  = node->xmlChildrenNode;
    while (cur != nullptr) {
        ...
        HDF_LOGD("%{public}s: ParsePollingNode groupName: %{public}s, interval: %{public}d",
        ...
        for (auto subNode = cur->children; subNode; subNode = subNode->next) {
            if (!xmlStrcmp(subNode->name, BAD_CAST"thermal_zone")) {
                ...
                HDF_LOGD("%{public}s: ParsePollingNode ParsePollingNodetztype: %{public}s, \
                    replace: %{public}s", __func__, tz.type.c_str(), tz.replace.c_str());
                vXtz.push_back(tz);
                ...
                HDF_LOGD("%{public}s: ParsePollingNode tntype: %{public}s, path: %{public}s",
