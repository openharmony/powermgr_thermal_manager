...
#ifndef ACTION_SOCPERF_RESOURCE_H
#define ACTION_SOCPERF_RESOURCE_H
 
#include "soc_action_base.h"
 
namespace OHOS {
namespace PowerMgr {
class ActionSocPerfResource : public IThermalAction, public SocActionBase {
public:
    ActionSocPerfResource(const std::string& actionName);
    ~ActionSocPerfResource() = default;
...
private:
    ...
    int32_t resId_ {0};
    uint32_t fallbackValue_ {0};
    uint32_t lastValue_ {0};
    std::vector<uint32_t> valueList_;
};
} // namespace PowerMgr
} // namespace OHOS
#endif // ACTION_SOCPERF_RESOURCE_H